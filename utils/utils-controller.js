...
function checkPayload(payload, schema, operationMode) {
  ...
  for (let payloadProp in payload) {
    ...
    let allowedTypes = schemaProp.type ? schemaProp.type.split('|'):[];
    allowedTypes = allowedTypes.map((x)=> x.trim());
    if(allowedTypes.length > 0 && !allowedTypes.includes(typeof payloadValue))
        throw new AppError(
          400,
          `typeof property '${payloadProp}' is not valid. expected ${schemaProp.type} but got ${typeof payloadValue}`
        );

    // else (schemaProp.type !== typeof payloadValue)
    //   throw new AppError(
    //     400,
